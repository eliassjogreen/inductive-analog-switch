...
class SwitchInductive(Footprint):
  ...
  def __init__(
    ...
    self.name = ("SW_L_" + ("LED_" if led else "") + "Inductive_" +
                 switch.switch_name.replace(" ", "_"))
    self.description = (switch.switch_name.capitalize() +
                        " keyswitch with inductor" +
                        (" and holes for an LED" if led else ""))
    self.tags = (switch.switch_name +
                 " switch keyswitch keyboard inductive inductor" +
                 (" LED" if led else ""))
